<div class="modal fade" id="edit-offline-donation-modal-form-{{ $donation->id }}" tabindex="-1" role="dialog" aria-labelledby="modal-form" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title">Edit Donasi Offline</h6>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form role="form" action="{{ route('transaction.donations.offline-donation.update', $donation->id) }}" method="post">
                @csrf
                @method('put')
                @if ($errors->any())
                    <script>
                        ...
                        <div class="card-body p-3">
                            <div class="row">
                                <div class="col-12 col-md-6">
                                    ...
                                    <select name="m_user_id"
                                        ...
                                        @foreach($donors as $donor)
                                            @if(old('m_user_id', $donation->m_user_id) == $donor->id)
                                ...
                                <div class="col-12 col-md-6">
                                    ...
                                    <select name="m_fundraising_program_id"
                                        ...
                                        @foreach($fundraisingPrograms as $fundraisingProgram)
                                            @if(old('m_fundraising_program_id', $donation->m_fundraising_program_id) == $fundraisingProgram->id)
                                ...
                                <div class="col-12">
                                    ...
                                    <input type="text" class="form-control form-control-sm @error('amount') is-invalid @enderror inputRupiah" name="amount" value="{{ old('originalAmount', isset($donation->amount) ? number_format($donation->amount, 0, ',', '.') : session('originalAmount')) }}">
                                    @error('amount')
                                    <div class="invalid-feedback text-xxs ms-1">
                                        {{ $message }}
                                    </div>
